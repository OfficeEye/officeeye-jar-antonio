       ...
       if (verificacaoLogin){
           ...
           if (maquinaFuncionario.isEmpty()){
               ...
               TimerTask task = new TimerTask() {
                   ...
                   public void run() {
                       if (verificacaoLogin) {
                           //Rede - Espaço disponivel
                            //Nome da maquina, nome do wifi, serviço em segundo plano
                           con.update("INSERT INTO customizacao (nomeMaquina, nomeWifi, servico) VALUES (?, ?, ?)",
                                 looca.getRede().getParametros().getHostName(), looca.getRede().getGrupoDeInterfaces().getInterfaces().get(0).getNomeExibicao(), looca.getGrupoDeServicos().getServicosAtivos().get(0).getNome());
